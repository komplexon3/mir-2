...
func UponGetChainRequest(m dsl.Module, handler func(requestId string, sourceModule stdtypes.ModuleID, endBlockId uint64, sourceBlockIds []uint64) error) {
...
func UponRegisterCheckpoint(m dsl.Module, handler func(blockId uint64, state *types3.State) error) {
	UponEvent[*types.Event_RegisterCheckpoint](m, func(ev *types.RegisterCheckpoint) error {
		return handler(ev.BlockId, ev.State)
	})
}

func UponGetChainToHeadRequest(m dsl.Module, handler func(sourceModule stdtypes.ModuleID) error) {
	UponEvent[*types.Event_GetChainToHeadRequest](m, func(ev *types.GetChainToHeadRequest) error {
		return handler(ev.SourceModule)
	})
}

func UponGetChainToHeadResponse(m dsl.Module, handler func(chain []*types2.Block, checkpointState *types3.State) error) {
...
func UponInitBlockchain(m dsl.Module, handler func(initialState *types3.State) error) {
