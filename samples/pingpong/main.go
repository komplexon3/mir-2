...
func main() {
	...
	if *debug {
		...
		if flag.NArg() > 0 {
			ownID = stdtypes.NodeID(flag.Arg(0))
		} else {
			fmt.Println("Node ID must be provided in debug mode")
			os.Exit(1)
		}
		interceptor, err = debugger.NewWebSocketDebugger(ownID, *debugPort, logging.ConsoleTraceLogger) // replace ownID with port number
		if err != nil {
			panic(err)
		}
	} else {
	...
	trantorParams := trantor.DefaultParams(membership)
	transport, err := grpc.NewTransport(trantorParams.Net, ownID, membership.Nodes[ownID].Addr, logging.ConsoleWarnLogger, nil)
	if err != nil {
		panic(err)
	}
