...
func (b *Broadcast) checkEmitFinal() error {
	b.logger.Log(logging.LevelDebug, "check emit")
	for _, instance := range b.ourInstances {
		b.logger.Log(logging.LevelDebug, "check emit", "instance", instance.broadcastID)
		if len(instance.echoSigs) > (b.params.GetN()+b.params.GetF())/2 && !instance.sentFinal {
			b.logger.Log(logging.LevelDebug, "yes for", "instance", instance.broadcastID)
